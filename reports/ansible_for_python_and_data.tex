...
\begin{frame}{What To Do}
  \framesubtitle{Adapted from Michael Feathers}
    \begin{itemize}
      \item \alert<1>{\textbf{Discovery} Log behavior}
      \item \alert<2>{\textbf{Exploration} Scripted investigation}
      \item \alert<3>{\textbf{Automating} Test harnesses (or clamps)}
      \item \alert<4>{\textbf{Deconstruction} Separate the target code}
      \item \alert<5>{\textbf{Enable} Switching with a feature flag}
      \item \alert<6>{\textbf{New Code} Side by side rewrite}
    \end{itemize}
  \note<1>{
